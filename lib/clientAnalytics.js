...
const QUERY_DEFINITIONS = {
  total_chats_v1: {
    defaultRangeDays: 30,
    queryText: `| where clientId == "clientID"
...
async function executeQuery(queryName, clientId, startDate, endDate) {
  ...
  if (TEST_MODE) {
    ...
    // Original logic - always use current server time as endTime to capture latest data
    startTime = startDate.toISOString();
    endTime = now.toISOString(); // Always "right now" to include data arriving after page load
  }
  ...
  // Dataset is in the URL (passed to axiomClient), not in the query text
