...
| where route == "/chat"
| where todatetime(timestamp) >= datetime("start_time") and todatetime(timestamp) <= datetime("end_time")
...
async function executeQuery(queryName, clientId, startDate, endDate) {
  ...
  // Ensure endTime is "now" if endDate is in the future
  const now = new Date();
  const effectiveEndDate = endDate > now ? now : endDate;
  const startTime = startDate.toISOString();
  const endTime = effectiveEndDate.toISOString();
