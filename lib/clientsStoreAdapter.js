...
// options: { actorUserId, actorEmail, actorRole, requestId }
async function writeClientConfigAtomic(clientId, config, updatedBy = null, options = {}) {
  if (useSupabase && supabaseStore) {
    // Supabase is active - use it exclusively (supports audit via options)
    return await supabaseStore.writeClientConfigAtomic(clientId, config, updatedBy, options);
  }
  // Filesystem fallback (only when Supabase is not available, doesn't support options)
