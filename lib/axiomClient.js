...
async function runQuery({ queryText, params = {}, dataset = null }) {
  ...
  try {
    // Construct API endpoint - Axiom uses /v1/datasets/_apl?format=tabular for APL queries
    // The dataset is specified within the APL query text using backticks (e.g., `advantum-prod-log`)
    const endpoint = `${normalizedApiUrl}/v1/datasets/_apl?format=tabular`;
    
    // NOTE: startTime/endTime are NOT included in the request body
    // Time filtering is done via explicit APL where clauses (e.g., | where _time >= datetime(...))
    // This avoids issues with Axiom API interpretation of body params vs APL time filters
    const startTime = params.start_time ? (typeof params.start_time === "string" ? params.start_time : new Date(params.start_time).toISOString()) : null;
    const endTime = params.end_time ? (typeof params.end_time === "string" ? params.end_time : new Date(params.end_time).toISOString()) : null;

    // Prepare request body - only include "apl" key
    // Time filtering is done within the APL query text itself using where _time >= datetime(...)
    const requestBody = {
      apl: queryText,
    };
    ...
    logAxiomEvent("debug", "axiom_query_request", {
      ...
      requestBodyShape: {
        hasApl: "apl" in requestBody,
        hasStartTime: false, // Time filtering is done in APL query, not request body
        hasEndTime: false,   // Time filtering is done in APL query, not request body
      },
      ...
      note: "Time filtering via APL where clause, not API body params",
    });
    ...
    if (response.data && response.data.tables && Array.isArray(response.data.tables) && response.data.tables.length > 0) {
      const firstTable = response.data.tables[0];
      if (firstTable.rows && Array.isArray(firstTable.rows) && firstTable.rows.length > 0) {
        firstRowKeys = Object.keys(firstTable.rows[0]);
      }
    }
    ...
    logAxiomEvent("debug", "axiom_query_response_shape", {
      ...
      hasTables: response.data && "tables" in response.data,
      tablesLength: response.data && response.data.tables ? response.data.tables.length : 0,
    });
    ...
    const endpointPath = `${normalizedApiUrl}/v1/datasets/_apl?format=tabular`;
 ...
 * Runs: `advantum-prod-log` | take 1 | project _time, timestamp
...
async function diagnoseTimestampField() {
  ...
  const diagnosticQuery = `\`${dataset}\` | take 1 | project _time, timestamp`;
...
/**
 * Smoke test: Verify APL query syntax with a simple query
 * Runs: `advantum-prod-log` | where event == "request_end" | take 1
 * @returns {Promise<object>} { success: boolean, rowsReturned: number, sampleRow: object }
 */
async function smokeTestQuery() {
  const dataset = process.env.AXIOM_DATASET || "advantum-prod-log";
  const smokeQuery = `\`${dataset}\` | where event == "request_end" | take 1`;
