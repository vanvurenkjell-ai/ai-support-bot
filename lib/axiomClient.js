...
async function runQuery({ queryText, params = {}, dataset = null }) {
  ...
  try {
    // Construct API endpoint - global APL endpoint
    // Dataset is specified IN the APL query using ['dataset-name'] syntax
    const endpoint = `${normalizedApiUrl}/v1/datasets/_apl?format=tabular`;

    // Time filtering via startTime/endTime in request body
    ...
    // Prepare request body - APL query + time range params
    // Note: APL query should NOT include dataset reference (it's in the URL)
    const requestBody = {
      apl: queryText,
    };

    // Include startTime/endTime in request body for time filtering
    if (startTime) {
    ...
    logAxiomEvent("info", "axiom_query_request", {
      ...
      aplQuerySnippet: queryTextSnippet,
      aplQueryLength: queryText.length,
      startTime: startTime || null,
      endTime: endTime || null,
      note: "Dataset in APL query using ['dataset-name'] syntax, time filtering via request body params",
    });
 ...
 * Runs: ['dataset-name'] | take 1 | project _time, timestamp
...
async function diagnoseTimestampField() {
  ...
  try {
    // Global APL endpoint - dataset is in the query
    const endpoint = `${normalizedApiUrl}/v1/datasets/_apl?format=tabular`;
    const headers = {
      "Authorization": `Bearer ${AXIOM_API_TOKEN}`,
      "Content-Type": "application/json",
 ...
 * Runs: ['dataset-name'] | where event == "request_end" | take 1
