...
async function runQuery({ queryText, params = {}, dataset = null }) {
  ...
  try {
    ...
    // The dataset is specified within the APL query text itself (e.g., ['advantum-prod-log'])
    const endpoint = `${normalizedApiUrl}/v1/datasets/_apl?format=tabular`;
    
    // Ensure startTime/endTime are RFC3339 ISO strings with Z (not Date objects)
    ...
    // Prepare request body - MUST use exact keys: "apl", "startTime", "endTime"
    // Axiom API format: { apl: "query text", startTime?: string, endTime?: string }
    const requestBody = {
      apl: queryText,
    };
    
    // Only include startTime/endTime if provided (omit entirely if not provided)
    if (startTime) {
